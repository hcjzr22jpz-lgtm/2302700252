...
describe('Staff routes', () => {
  ...
  test('customer accounts cannot access staff products', async () => {
    const agent = await loginAgent('customer@x.com', 'pass');
    const res = await agent.get('/staff/products');
    expect(res.status).toBe(403);
  });

  test('staff accounts can view product management', async () => {
  ...
  test('staff can create a product', async () => {
    const agent = await loginAgent('staff2@x.com', 'pass', 'staff');
    const res = await agent.post('/staff/products').send(
      'name=Route+Hat&price=18.75&image=/images/red-tshirt.svg&category=Apparel&type=Hat&badge=New&desc=Route+test'
    );
    expect(res.status).toBe(302);
    expect(res.headers.location).toBe('/staff/products?created=1');
    expect(Product.getAll().some(p => p.name === 'Route Hat')).toBe(true);
  });

  test('staff can create an order for a customer', async () => {
    const agent = await loginAgent('staff3@x.com', 'pass', 'staff');
    const customer = Account.add({ name: 'Buyer', email: 'buyer-route@x.com', password: 'pass', address: '10 St' });
    const res = await agent.post('/staff/order').send(`customerId=${customer.id}&qty_1=1`);
    expect(res.status).toBe(200);
    expect(res.text).toMatch(/ORD-|Order Summary/i);
    expect(Order.getAll()).toHaveLength(1);
    expect(Order.getAll()[0]).toMatchObject({ userId: customer.id, channel: 'staff' });
  });
});
