...
describe('requireStaff', () => {
  ...
  test('returns 403 for customer accounts', () => {
    const req = { session: { user: { id: 1, role: 'customer' } } };
    const res = mockRes();
    const next = jest.fn();
    authCtrl.requireStaff(req, res, next);
    expect(res.status).toHaveBeenCalledWith(403);
    expect(res.send).toHaveBeenCalledWith('Staff access required');
    expect(next).not.toHaveBeenCalled();
  });

  test('calls next for staff accounts', () => {
    const req = { session: { user: { id: 2, role: 'staff' } } };
    const res = mockRes();
    const next = jest.fn();
    authCtrl.requireStaff(req, res, next);
    expect(next).toHaveBeenCalled();
