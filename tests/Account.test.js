...
describe('Account.add()', () => {
  ...
  test('can create staff accounts when role is provided', () => {
  ...
  test('throws when role is invalid', () => {
    expect(() => Account.add({ ...SAMPLE, role: 'manager' })).toThrow('Invalid role.');
  });
...
describe('Account role classification', () => {
  test('separates customers and staff', () => {
    Account.add(SAMPLE);
    Account.add({ ...SAMPLE, email: 'staff@example.com', role: 'staff' });
    expect(Account.getCustomers()).toHaveLength(1);
    expect(Account.getStaff()).toHaveLength(1);
  });

  test('treats missing role as customer', () => {
    fs.writeFileSync(dataFile, JSON.stringify([{ id: 1, email: 'legacy@example.com' }]));
    expect(Account.getCustomers()).toHaveLength(1);
  });
});
...
describe('Account.update()', () => {
  ...
  test('normalizes role during update', () => {
    const acc = Account.add(SAMPLE);
